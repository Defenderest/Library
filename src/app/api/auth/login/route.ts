...
import { execute, queryFirst } from "@/lib/db/raw";
import { prisma } from "@/lib/prisma";

export const dynamic = "force-dynamic";

type LoginCustomerRow = {
  customerId: number;
  firstName: string;
  lastName: string;
  email: string;
...
export async function POST(request: Request) {
  ...
  const customer = await queryFirst<LoginCustomerRow>(prisma, "auth/get_customer_login_by_email", [email]);
  ...
  if (passwordHashNeedsUpgrade(customer.passwordHash)) {
    ...
    if (upgradedHash.length > 0) {
      await execute(prisma, "auth/update_customer_password_hash", [customer.customerId, upgradedHash]);
    }
  }
