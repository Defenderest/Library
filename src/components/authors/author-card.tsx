...
export function AuthorCard({ author, href = `/authors/${author.authorId}`, className }: AuthorCardProps) {
  ...
  return (
    <Link
      href={href}
      className={cn(
        "group relative block h-[280px] w-full overflow-hidden rounded-sharp border border-app-border-light bg-app-card p-5 text-center transition-[transform,border-color,background-color,box-shadow] duration-normal hover:-translate-y-[5px] hover:border-app-border-hover hover:bg-app-hover hover:shadow-[0_20px_48px_rgba(0,0,0,0.18)] focus-visible:-translate-y-[5px] focus-visible:border-app-border-hover focus-visible:bg-app-hover focus-visible:shadow-[0_20px_48px_rgba(0,0,0,0.18)]",
        className,
      )}
    >
      <span className="pointer-events-none absolute inset-x-10 top-0 h-px bg-[linear-gradient(90deg,transparent_0%,var(--color-card-top-glow)_50%,transparent_100%)] opacity-0 transition duration-slow group-hover:opacity-100" />

      <div className="app-frame-surface relative mx-auto h-[120px] w-[120px] overflow-hidden rounded-full border border-app-border-light shadow-[0_12px_24px_rgba(0,0,0,0.14)] transition-[transform,border-color,box-shadow] duration-slow group-hover:-translate-y-[3px] group-hover:border-app-border-hover group-hover:shadow-[0_18px_34px_rgba(0,0,0,0.18)]">
        ...
        {imageSource ? (
          ...
          <div className="app-frame-inner-surface flex h-full w-full items-center justify-center font-display text-4xl text-app-secondary">
            ?
          </div>
        )}
      </div>
